#!/bin/bash
# Handler: Jira Close Issue
# Transitions issue to Done/Closed state with optional comment
# CRITICAL: This actually closes the issue (fixes v1.x bug)

set -euo pipefail

# Check arguments
if [ $# -lt 1 ]; then
    echo "Usage: $0 <issue_key> [close_comment] [work_id]" >&2
    echo "  Example: $0 PROJ-123 'Fixed in PR #456' 'faber-abc123'" >&2
    exit 2
fi
...
# Step 2: Find transition to "Done" or "Closed" state
# Load target states from config if available
CONFIG_FILE=".faber/plugins/work/config.json"
if [ -f "$CONFIG_FILE" ]; then
    DONE_STATE=$(jq -r '.handlers["work-tracker"].jira.states.done // "Done"' "$CONFIG_FILE")
    CLOSED_STATE=$(jq -r '.handlers["work-tracker"].jira.states.closed // "Closed"' "$CONFIG_FILE")
else
    DONE_STATE="Done"
    CLOSED_STATE="Closed"
fi

# Find transition that leads to Done or Closed
transition_id=$(echo "$transitions_body" | jq -r --arg done "$DONE_STATE" --arg closed "$CLOSED_STATE" \
  '.transitions[] | select(.to.name == $done or .to.name == $closed or .to.name == "Resolved") | .id' | head -1)

if [ -z "$transition_id" ] || [ "$transition_id" = "null" ]; then
    echo "Error: No valid transition to close issue $ISSUE_KEY" >&2
...
# If close comment provided, include it in the transition
if [ -n "$CLOSE_COMMENT" ]; then
    # Build ADF for comment
    # For MVP, use markdown-to-adf converter
...
if [ "$http_code" -ne 204 ] && [ "$http_code" -ne 200 ]; then
    echo "Error: Failed to close issue $ISSUE_KEY (HTTP $http_code)" >&2

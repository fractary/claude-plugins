#!/bin/bash
# Handler: Linear Add Label
# Adds a label to an issue (requires UUID lookup)
...
# Step 2: Add label to issue
read -r -d '' ADD_LABEL_MUTATION <<'EOF' || true
...
if echo "$RESPONSE" | jq -e '.errors' > /dev/null 2>&1; then
    ...
    echo "Error: Failed to add label: $ERROR_MSG" >&2
    exit 1
fi

SUCCESS=$(echo "$RESPONSE" | jq -r '.data.issueAddLabel.success')

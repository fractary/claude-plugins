...
**File**: `.fractary/specs/logs-remediation-{timestamp}.md`
...
- Stored in `.fractary/specs/` or Spec Manager's location
...
**Storage**: `.fractary/audit/` (temporary working directory)
...
Step 7: Summary
  ...
  📁 Outputs:
     ...
     - Remediation Spec (persistent): .fractary/specs/logs-remediation-20250115-143022.md
     - Discovery Data (temp): .fractary/audit/discovery-*.json
...
├── .fractary/
│   ├── audit/                           # Temporary working directory
│   │   ├── discovery-logs.json
│   │   ├── discovery-vcs-logs.json
│   │   ├── discovery-patterns.json
│   │   └── discovery-storage.json
│   └── specs/                           # Persistent specs (committed to VCS)
│       └── logs-remediation-20250115-143022.md
```
...
**Location**: `.fractary/specs/logs-remediation-{timestamp}.md`
...
**Storage & Retention**:
- Generated by **Spec Manager agent**
- Stored in `.fractary/specs/` or Spec Manager's managed location
- Committed to version control (persistent)
...
- ✅ Timestamped for multiple audits
...
2. **Review the remediation spec** (understand how to change it)
   ```bash
   # Find latest spec (or check Spec Manager location)
   ls -lt .fractary/specs/
   cat .fractary/specs/logs-remediation-20250115-143022.md
   ```

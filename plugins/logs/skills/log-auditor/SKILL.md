...
- **Remediation Spec** (Step 7): Persistent action plan for project
  - Location: `.fractary/specs/logs-remediation-{timestamp}.md` (created by Spec Manager)
...

{
  ...
  "output_path": ".fractary/specs/logs-remediation-{timestamp}.md"
}
```

**Important Notes on Spec Manager Invocation:**
- Use natural language agent invocation (not a tool call)
- The spec-manager agent will create timestamped spec in `.fractary/specs/`
...
Generate markdown specification directly following similar structure but in plain markdown format. Write directly to `.fractary/specs/logs-remediation-{timestamp}.md`.

**Note on Spec Location**: The Spec Manager may place the spec in its own managed location (e.g., `.fractary/specs/` or similar). Follow Spec Manager conventions.
...
📋 OUTPUT DOCUMENTS
  ...
  2. Remediation Spec: .fractary/specs/logs-remediation-{timestamp}.md
...
💡 NEXT STEPS
  ...
  2. Review remediation spec: .fractary/specs/logs-remediation-{timestamp}.md
...
Outputs:
  ...
  • Remediation Spec (persistent): .fractary/specs/logs-remediation-{timestamp}.md
  • Discovery Data: {output_dir}/discovery-*.json
───────────────────────────────────────────────────────────
...
- **Remediation spec generated**: `.fractary/specs/logs-remediation-{timestamp}.md` with:
...
```json
{
  ...
  "result": {
    ...
    "outputs": {
      ...
      "remediation_spec": ".fractary/specs/logs-remediation-20250115-143022.md",
      "discovery_files": [
